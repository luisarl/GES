...
function CargarAlmacenUsuario()
{
    ...
    $("#tabla_almacen>tbody").append("<tr><td id='id_empresas'>"+empresa+"</td><td id='id_almacen'>"+almacen+"</td><th> <button type='button' class='borrar btn btn-danger btn-sm'><i class='fa fa-trash'></i></button> </th></tr>");
    $('#_almacenes').val('');
    $('#_empresas').val(0);
...
//         let fila = {
//             id_empresa: e.querySelector('#_empresas').innerText,
//             id_almacen: e.querySelector('#_almacenes').innerText,
//         };
...
function CapturarDatosAlmacenes()
{
    ...
    for(var i = 0; i < filas; i++)
    {
        for(var j = 0; j < 1; j ++)
        {
            arreglo.push([celda[acumulador1].innerHTML.trim().split('')[0], celda[acumulador2].innerHTML.trim().split('')[0]]);
