...
@section('titulo_pagina', 'Editar Vehiculo')
...
<div class="card">
    ...
    <div class="card-block">
        ...
        <form method="POST" action=" {{ route('vehiculos.update', $vehiculo->id_vehiculo) }}" enctype="multipart/form-data">
            @csrf @method("put")
            ...
            <label class="col-sm-2 col-form-label">Placa</label>
                <div class="col-sm-4">
                    <input type="text" class="form-control @error('placa_vehiculo') is-invalid @enderror" name="placa_vehiculo"
                       value="{{old('placa_vehiculo', $vehiculo->placa_vehiculo ?? '')}}" placeholder="" readonly>
                </div>
                <label class="col-sm-2 col-form-label">Marca</label>
                <div class="col-sm-4">
                    <input type="text" class="form-control @error('marca_vehiculo') is-invalid @enderror" name="marca_vehiculo"
                       value="{{old('marca_vehiculo', $vehiculo->marca_vehiculo ?? '')}}" placeholder="Marca Vehiculo">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-4">
                    <input type="text" class="form-control @error('modelo_vehiculo') is-invalid @enderror" name="modelo_vehiculo"
                       value="{{old('modelo_vehiculo', $vehiculo->modelo_vehiculo ?? '')}}" placeholder="Modelo Vehiculo">
                </div>
                <label class="col-sm-2 col-form-label">Descripcion</label>
                <div class="col-sm-4">
                    <textarea rows="3" cols="3" class="form-control @error('descripcion') is-invalid @enderror" name="descripcion"
                       value="" placeholder="Breve Descripcion del Vehiculo">{{old('descripcion', $vehiculo->descripcion ?? '')}}</textarea>
                </div>
            </div>
            <div class="form-group row">
                    ...
                    <div class="form-check form-check-inline">
                        <label class="form-check-label">
                            <input class="form-check-input" type="radio" name="activo"
                                value="SI" @if($vehiculo->activo == 'SI') checked @endif> Activo </label>
                    </div>
                    <div class="form-check form-check-inline">
                        <label class="form-check-label">
                            <input class="form-check-input" type="radio" name="activo"
                                value="NO" @if($vehiculo->activo == 'NO') checked @endif> Inactivo </label>
                    </div>
                </div>
            </div>
