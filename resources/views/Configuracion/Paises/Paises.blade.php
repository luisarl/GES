...
@include('Configuracion.Paises.PaisesDestroy')


<div class="card">
    <div class="card-header">
        <h5>Listado de los Paises</h5>
            <a class="btn btn-primary " title="Nuevo" data-toggle="modal" data-target="#modal-migracion" href="#">
            <i class="fa fa-refresh"></i> Migrar</a>
        @can('conf.paises.crear')
            <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('paises.create') !!}">
                    ...
                    <tr>
                        ...
                        <td class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right b-none contact-menu"> 
                                @can('conf.paises.editar')
                                    <a class="dropdown-item" href=" {{ route('paises.edit', str_replace(' ', '', $pais->id_pais)) }}"><i class="fa fa-edit"></i>Editar</a>                            
                                @endcan 
                                @can('conf.paises.eliminar')
                                    <a class="dropdown-item" data-id="{{str_replace(' ', '', $pais->id_pais) }}" data-nombre="{{ $pais->nombre_pais }}"  data-toggle="modal" data-target="#modal-eliminar" href="#!"><i class="fa fa-trash"></i>Borrar</a>
                                @endcan 
                            </div>
                        </td>
                    </tr>
   ...
   <script src="{{ asset('libraries\bower_components\datatables.net\js\jquery.dataTables.min.js') }}"></script>
    ...
    <script>
        $('#modal-eliminar').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-body h5').text('Desea Eliminar el Pais ' + nombre + ' ?') //cambia texto en el body del modal
        })
    </script>
