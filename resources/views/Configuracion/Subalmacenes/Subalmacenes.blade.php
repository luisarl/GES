...
@section('menu_pagina')
    <ul class="breadcrumb-title">
        ...
        <li class="breadcrumb-item"><a href="#!">Sub Almacenes</a> </li>
    </ul>
@endsection
...
@section('contenido')
    ...
    @include('Configuracion.Subalmacenes.SubalmacenesDestroy')
    <!-- Scroll - Vertical table end -->
    <!-- Scroll - Vertical, Dynamic Height table start -->
    <div class="card">
        <div class="card-header">
            <h5>Listado de Sub Almacenes</h5>
            @can('conf.subalmacenes.crear')
                <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('subalmacenes.create') !!}">
                ...
                <div class="dt-responsive table-responsive">
                    ...
                    <tbody>
                        ...
                        @foreach ($subalmacenes as $subalmacen)
                            <tr>
                                <td>{{ $subalmacen->codigo_subalmacen }}</td>
                                <td>{{ $subalmacen->nombre_subalmacen }}</td>
                                <td>{{ $subalmacen->almacen->nombre_almacen }} </td>
                                <td>
                                    @php
                                        $texto = $subalmacen->descripcion_subalmacen;
                                        $descripcion_subalmacen = wordwrap($texto, 40, '<br>', false);
                                    @endphp
                                ...
                                <td class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                        @can('conf.subalmacenes.editar')
                                            <a class="dropdown-item"
                                                href="{{ route('subalmacenes.edit', $subalmacen->id_subalmacen) }}"><i
                                                    class="fa fa-edit"></i>Editar</a>
                                        @endcan
                                        @can('conf.subalmacenes.eliminar')
                                            <a class="dropdown-item" data-id="{{ $subalmacen->id_subalmacen }}"
                                                data-nombre="{{ $subalmacen->nombre_subalmacen }}" data-toggle="modal"
...
@section('scripts')
    ...
    <script>
        $('#modal-eliminar').on('show.bs.modal', function(event) {
            ...
            var modal = $(this)
            modal.find('.modal-body h5').text('Desea Eliminar el subalmacen ' + nombre +
