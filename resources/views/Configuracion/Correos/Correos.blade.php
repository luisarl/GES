...
@section('menu_pagina')
    <ul class="breadcrumb-title">
        ...
        <li class="breadcrumb-item"><a href="#!">Correos</a> </li>
    </ul>
@endsection
...
@include('Configuracion.Correos.CorreosDestroy')


<div class="card">
    <div class="card-header">
        <h5>Listado del Usuario</h5>
        @can('conf.usuarioscorreos.crear')
        <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('correos.create') !!}">
                ...
                <tbody>
                    @foreach ($correos as $correo)
                        <tr>
                            ...
                            <td>{{ $correo->nombre_destinatario }}</td>
                            <td class="dropdown">
                                <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown"
                                    aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog"
                                        aria-hidden="true"></i></button>
                                <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                    @can('conf.usuarioscorreos.editar')
                                        <a class="dropdown-item" href="{{ route('correos.edit', $correo->id_correo) }}"><i class="fa fa-edit"></i>Editar</a>
                                    @endcan
                                    @can('conf.usuarioscorreos.eliminar')         
                                        <a class="dropdown-item" data-id="{{ $correo->id_correo }}" data-nombre="{{ $correo->proceso }}" data-toggle="modal" data-target="#modal-eliminar" href="#!"><i class="fa fa-trash"></i>Borrar</a>
                                    @endcan
                                </div>
                            </td>
                        </tr>
...
@section('scripts')
    ...
    <script>
        $('#modal-eliminar').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-body h5').text('Desea Eliminar el Proce de Correo ' + nombre + ' ?') //cambia texto en el body del modal
        })
    </script>
