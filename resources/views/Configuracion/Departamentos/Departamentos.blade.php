...
@include('Configuracion.Departamentos.DepartamentosDestroy')
...
<div class="card">
    <div class="card-header">
        ...
        @can('conf.departamentos.crear')
        <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('departamentos.create') !!}">
            ...
            <table id="datatable" class="table table-striped table-bordered nowrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$departamento->correo}}</td>
                        <td>{{$departamento->aplica_servicios}}</td>
                        <td class="dropdown">
                            <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog" aria-hidden="true"></i></button>
                            <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                @can('conf.departamentos.editar')                                  
                                    <a class="dropdown-item" href=" {{ route('departamentos.edit', $departamento->id_departamento) }}"><i class="fa fa-edit"></i>Editar</a>
                                @endcan
                                @can('conf.departamentos.eliminar')   
                                    <a class="dropdown-item" data-id="{{ $departamento->id_departamento }}" data-nombre="{{ $departamento->nombre_departamento }}"  data-toggle="modal" data-target="#modal-eliminar" href="#!"><i class="fa fa-trash"></i>Borrar</a>
                                @endcan
                        </td>
                    </tr>
                    @endforeach
   ...
   <script src="{{ asset('libraries\bower_components\datatables.net\js\jquery.dataTables.min.js') }}"></script>
    ...
    <script>
        $('#modal-eliminar').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-body h5').text('Desea Eliminar el Departamento ' + nombre + ' ?') //cambia texto en el body del modal
        })
    </script>
