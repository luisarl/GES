...
@section('titulo', 'Solicitud de Resguardo')

@section('titulo_pagina', 'Editar Solicitud de Resguardo')
...
@include('Resguardo.Solicitudes.SolicitudesAprobar')
@include('Resguardo.Solicitudes.SolicitudesProcesar')
@include('Resguardo.Solicitudes.SolicitudesImprimirEtiquetas')

<div class="card">
    <div class="card-header">
        <h4 class="sub-title"><strong>DETALLE</strong>
        {{-- BOTON IMPRIMIR --}}  
            <a href="{{ route('resgimprimirsolicitudresguardo', $solicitud->id_solicitud_resguardo) }}" target="_blank" type="button"
                class="btn btn-primary btn-sm" title="Imprimir">
                <i class="fa fa-print fa-2x"></i> </a>

            {{-- BOTON IMPRIMIR ETIQUETAS --}}  
            <a class="btn btn-primary btn-sm" data-id="{{ $solicitud->id_solicitud_resguardo}}"
                data-nombre="{{ $solicitud->id_solicitud_resguardo }}" data-toggle="modal"
                data-target="#modal-imprimir" href="#!" title="Imprimir Etiquetas">
                <i class="fa fa-print fa-2x"></i>  <i class="fa fa-tags fa-2x"></i></a>
        </h4>
    </div>
    <div class="card-block">
        <form method="POST" action="{{ route('resgsolicitudes.update', $solicitud->id_solicitud_resguardo) }}">
            @csrf
            @method('put')
            <div class="form-group row">
                ...
                <div class="col-sm-4">
                    <input type="text" name="ubicacion_actual" id="ubicacion_actual" class="form-control @error('ubicacion_actual') is-invalid @enderror" value="{{ old('destino', $solicitud->ubicacion_actual) }}" {{$procesado}}>
                </div>
            </div>
            <div class="form-group row">
            ...
            <div class="form-group row" id="buscar_articulo">     
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-2 col-lg-2">
                        ...
                        <br>
                        <textarea name="comentario" id="comentario" class="form-control" cols="5" rows="1"></textarea>
                    </div>
                    <div class="col-md-1 col-lg-1">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered" id="tablaarticulos">
                    <thead>
                        <tr>
                            <th style="visibility:collapse; display:none;">Id Resguardo</th>
                            <th>Codigo</th>
                            <th>Articulo</th>
                            <th>Unidad</th>
                            <th>Presentacion</th>
                            <th>Cantidad</th>
                            <th>Estado</th>
                            <th style='visibility:collapse; display:none;'>Id Clasificacion</th>
                    ...
                    <tbody>
                        @foreach($articulos as $articulo)
                            <tr>
                                ...
                                <td id="equivalencia_unidad">{{number_format($articulo->equivalencia_unidad, 2)}}</td>
                                ...
                                <td id="observacion" contenteditable="true">{{$articulo->observacion}}</td>
                                <td>
                                    @if($solicitud->estatus != 'PROCESADO')
                                    <button type="button" onclick="EliminarDetalle({{ $articulo->id_resguardo }})"
            ...
            <input type="hidden" name="articulos" id="articulos">
                <div class="d-grid gap-2 d-md-block float-right">
                    ...
                    @if($solicitud->estatus == 'APROBADO')
                        @can('resg.solresguardo.procesar')
...
<script>
    ...
    var EliminarArticulo = "{{ url('resgeliminararticuloresguardo') }}"; 
