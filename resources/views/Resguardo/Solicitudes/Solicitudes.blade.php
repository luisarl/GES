...
@section('titulo_pagina', 'Solicitudes de Resguardo')
...
@include('Resguardo.Solicitudes.SolicitudesDestroy')

<div class="card">
    <div class="card-header">
        <h5>Listado de Solicitudes</h5>
        @can('resg.solresguardo.crear')
        <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('resgsolicitudes.create') !!}">
        ...
        <div class="dt-responsive table-responsive">
            <table id="datatable" class="table table-striped table-bordered nowrap" style="width: 100%">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$solicitud->id_solicitud_resguardo}}</td>
                        <td>{{$solicitud->fecha_creacion}}</td>
                        <td>{{$solicitud->creado_por}}</td>
                        <td>{{$solicitud->nombre_almacen}}</td>
                        <td>{{$solicitud->ubicacion_actual}}</td>
                        <td>
                        ...
                        <td class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                @can('resg.solresguardo.ver')
                                    <a class="dropdown-item" href="{{ route('resgsolicitudes.show', $solicitud->id_solicitud_resguardo) }}">
                                    <i class="fa fa-eye"></i>Ver</a>
                                @endcan
                               
                                @if ($solicitud->estatus != 'ANULADO')
                                @can('resg.solresguardo.editar')
                                <a class="dropdown-item"
                                    href="{{ route('resgsolicitudes.edit',$solicitud->id_solicitud_resguardo) }}">
                                    <i class="fa fa-edit"></i>Editar</a>
                                @endcan
                                @endif
                                @if ($solicitud->estatus == 'POR APROBACION' || $solicitud->estatus == 'APROBADO')
                                @can('resg.solresguardo.eliminar')
                                <a class="dropdown-item" data-id="{{ $solicitud->id_solicitud_resguardo}}"
...
<script>
    $('#modal-eliminar').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-body h5').text('Desea Anular la Solcitud de Resguardo ' + nombre + ' ?') //cambia texto en el body del modal
        })
</script>
