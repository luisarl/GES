...
@include('FichaTecnica.Grupos.GruposMigrate')
@include('FichaTecnica.Grupos.GruposDestroy')
...
<div class="card">
    <div class="card-header">
        ...
        @can('fict.grupos.migracion')
            <a class="btn btn-primary " title="Nuevo" data-toggle="modal" data-target="#modal-migracion" href="#">
            <i class="fa fa-refresh"></i> Migrar</a>
        @endcan
        @can('fict.grupos.crear')
        <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('grupos.create') !!}">
            ...
            <table id="datatable" class="table table-striped table-bordered nowrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                             @php
                                $texto = $grupo->descripcion_grupo;
                                $descripcion_grupo = wordwrap($texto, 40, "<br>", false);
                            @endphp
                        ...
                        <td class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                @can('fict.grupos.editar')
                                <a class="dropdown-item" href=" {{ route('grupos.edit', $grupo->id_grupo) }}"><i class="fa fa-edit"></i>Editar</a>
                                @endcan
                                @can('fict.grupos.eliminar')
                                <a class="dropdown-item" data-id="{{ $grupo->id_grupo }}" data-nombre="{{ $grupo->nombre_grupo }}"  data-toggle="modal" data-target="#modal-eliminar" href="#!"><i class="fa fa-trash"></i>Borrar</a>
                                @endcan
                            </div>
                        </td>
   ...
   <script src="{{ asset('libraries\bower_components\datatables.net\js\jquery.dataTables.min.js') }}"></script>
    ...
    <script>
        $('#modal-eliminar').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-body h5').text('Desea Eliminar el Grupo ' + nombre + ' ?') //cambia texto en el body del modal
        })
    </script>
