...
@include('FichaTecnica.Categorias.CategoriasMigrate')
@include('FichaTecnica.Categorias.CategoriasDestroy')
...
<div class="card">
    <div class="card-header">
        ...
        @can('fict.categorias.migracion')
            <a class="btn btn-primary " title="Nuevo" data-toggle="modal" data-target="#modal-migracion" href="#">
                <i class="fa fa-refresh"></i> Migrar</a>
        @endcan
        @can('fict.categorias.crear')
        <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('categorias.create') !!}">
            ...
            <table id="datatable" class="table table-striped table-bordered nowrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @php
                               $texto = $categoria->descripcion_categoria;
                               $descripcion_categoria = wordwrap($texto, 40, "<br>", false);
                           @endphp
                        ...
                        <td class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                @can('fict.categorias.editar')
                                <a class="dropdown-item" href="  {{ route('categorias.edit', $categoria->id_categoria) }}"><i class="fa fa-edit"></i>Editar</a>
                                @endcan
                                @can('fict.categorias.eliminar')
                                <a class="dropdown-item" data-id="{{ $categoria->id_categoria }}" data-nombre="{{ $categoria->nombre_categoria }}" data-toggle="modal" data-target="#modal-eliminar" href="#!"><i class="fa fa-trash"></i>Borrar</a>
                                @endcan
                            </div>
                        </td>
                    </tr>
...
<script>
    $('#modal-eliminar').on('show.bs.modal', function (event) {
        ...
        var modal = $(this)
        modal.find('.modal-body h5').text('Desea Eliminar La Categoria ' + nombre + ' ?') //cambia texto en el body del modal
    })
</script>
