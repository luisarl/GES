...
@section('titulo_pagina', 'Unidades de Medidas')
...
@section('menu_pagina')
    <ul class="breadcrumb-title">
        ...
        <li class="breadcrumb-item"><a href="#!">Unidades de Medidas</a> </li>
    </ul>
@endsection
...
@include('FichaTecnica.Unidades.UnidadesMigrate')
@include('FichaTecnica.Unidades.UnidadesDestroy')
    <!-- Scroll - Vertical table end -->
    <div class="card">
        <div class="card-header">
            <h5>Listados de Unidades</h5>
            @can('fict.unidades.migracion')
                <a class="btn btn-primary " title="Nuevo" data-toggle="modal" data-target="#modal-migracion" href="#">
                <i class="fa fa-refresh"></i> Migrar</a>
            @endcan
            @can('fict.unidades.crear')
            <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('unidades.create') !!}">
                ...
                <table id="datatable" class="table table-striped table-bordered nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$unidad->abreviatura_unidad}}</td>
                            <td>{{$unidad->clasificacion_unidad}}</td>
                            <td class="dropdown">
                                <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog" aria-hidden="true"></i></button>
                                <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                    @can('fict.unidades.editar')    
                                    <a class="dropdown-item" href="  {{ route('unidades.edit', $unidad->id_unidad) }}"><i class="fa fa-edit"></i>Editar</a>
                                    @endcan
                                    @can('fict.unidades.eliminar')
                                    <a class="dropdown-item" data-id="{{ $unidad->id_unidad }}" data-nombre="{{ $unidad->nombre_unidad }}"  data-toggle="modal" data-target="#modal-eliminar" href="#!"><i class="fa fa-trash"></i>Borrar</a>
                                    @endcan
                                </div>
                            </td>
                        </tr>
...
@section('scripts')
    ...
    <script>
        $('#modal-eliminar').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-body h5').text('Desea Eliminar la Unidad ' + nombre + ' ?') //cambia texto en el body del modal
        })
    </script>
