...
@include('ControlHerramientas.Subgrupos.SubgruposDestroy')
    <!-- Scroll - Vertical table end -->
    <!-- Scroll - Vertical, Dynamic Height table start -->
    <div class="card">
        <div class="card-header">
            <h5>Listado de SubGrupos</h5>
            @can('cnth.subgrupos.crear') 
                <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('cnthsubgrupos.create') !!}">
        ...
        <div class="card-block">
            <div class="dt-responsive table-responsive">
                <table id="datatable" class="table table-striped table-bordered nowrap">
                    ...
                    <tbody>
                        @foreach ($cnthsubgrupos as $subgrupo)
                        <tr>
                            <td>{{$subgrupo->codigo_subgrupo}}</td>
                            <td>{{$subgrupo->nombre_subgrupo}} </td>
                            <td>{{$subgrupo->descripcion_subgrupo}}</td>
                            <td>{{$subgrupo->grupo->nombre_grupo}}</td>
                            <td class="dropdown">
                                <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog" aria-hidden="true"></i></button>
                                <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                    @can('cnth.subgrupos.editar') 
                                        <a class="dropdown-item" href=" {{ route('cnthsubgrupos.edit', $subgrupo->id_subgrupo) }}"><i class="fa fa-edit"></i>Editar</a>
                                    @endcan 
                                    @can('cnth.subgrupos.eliminar') 
