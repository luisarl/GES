...
@section('titulo_pagina', 'Plantillas')
...
@include('ControlHerramientas.Plantillas.PlantillasDestroy')

<div class="card">
    <div class="card-header">
        <h5>Listado de Plantillas</h5>
        @can('cnth.plantillas.crear') 
            <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('plantillas.create') !!}">
            ...
            <table id="datatable" class="table table-striped table-bordered nowrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$plantilla->nombre_almacen}}</td>
                        <td class="dropdown">
                            <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown"
                                aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog"
                                    aria-hidden="true"></i></button>
                            <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                @can('cnth.plantillas.editar') 
                                    <a class="dropdown-item" href=" {{ route('plantillas.edit', $plantilla->id_plantilla) }}"><i
                                            class="fa fa-edit"></i>Editar</a>
                                @endcan
                                @can('cnth.plantillas.eliminar') 
                                    <a class="dropdown-item" data-id="{{ $plantilla->id_plantilla }}"
                                        data-nombre="{{ $plantilla->nombre_plantilla }}" data-toggle="modal"
...
<script>
    $('#modal-eliminar').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-body h5').text('Desea Eliminar la Plantilla ' + nombre + ' ?') //cambia texto en el body del modal
        })
</script>
