...
@include('SolicitudesServicios.Servicios.ServiciosDestroy')

<div class="card">
    <div class="card-header">
        <h5>Listado de Servicios</h5>
        @can('sols.servicios.crear') 
            <a class="btn btn-primary float-right" title="Nuevo" href="{!! route('servicios.create') !!}">
            ...
            <table id="datatable" class="table table-striped table-bordered nowrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @php
                            $texto = $servicio->descripcion_servicio;
                            $descripcion_servicio = wordwrap($texto, 40, "<br>", false);
                            @endphp

                            {!!$descripcion_servicio!!}
                        </td>
                        <td>{{$servicio->nombre_departamento}}</td>
                        <td class="dropdown">
                            <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown"
                                aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog"
                                    aria-hidden="true"></i></button>
                            <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                @can('sols.servicios.editar') 
                                    <a class="dropdown-item" href=" {{ route('servicios.edit', $servicio->id_servicio) }}"><i
                                            class="fa fa-edit"></i>Editar</a>
                                @endcan
                                @can('sols.servicios.eliminar') 
                                    <a class="dropdown-item" data-id="{{ $servicio->id_servicio }}"
                                        data-nombre="{{ $servicio->nombre_servicio }}" data-toggle="modal"
