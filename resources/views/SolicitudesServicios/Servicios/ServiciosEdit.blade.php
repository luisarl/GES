    ...
    <div class="card-block">
        ...
        <form method="POST" action=" {{ route('servicios.update', $servicio->id_servicio) }}" enctype="multipart/form-data">
               ...
               <div class="col-sm-10 @error('id_departamento') is-invalid @enderror">
                   ...
                   @foreach ($departamentos as $departamento)
                       <option value="{{ $departamento->id_departamento }}"
                           @if ($departamento->id_departamento == old('id_departamento', $servicio->id_departamento ?? '')) selected = "selected" 
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control @error('nombre_servicio') is-invalid @enderror" name="nombre_servicio" value=" {{old('nombre_servicio', $servicio->nombre_servicio ?? '')}}" placeholder="Ingrese el Nombre del Servicio">
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label">Descripción</label>
                <div class="col-sm-10">
                    <textarea rows="3" cols="3"class="form-control @error('descripcion_servicio') is-invalid @enderror" name="descripcion_servicio" placeholder="Ingrese la Descripción del Servicio">{{old('descripcion_servicio', $servicio->descripcion_servicio ?? '')}}</textarea>
                </div>
            </div>
