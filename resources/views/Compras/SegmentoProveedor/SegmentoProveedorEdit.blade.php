...
@section('menu_pagina')
    <ul class="breadcrumb-title">
        ...
        <li class="breadcrumb-item"><a href="{{ asset('segmentos/') }}">Segmentos del Proveedor</a>
        </li>
...
<div class="card">
    <div class="card-header">
        <h5>Editar el Segmento</h5>
    </div>
    <div class="card-block">
        <h4 class="sub-title"></h4>
        <form method="POST" action=" {{ route('segmentos.update', $segmentos->id_segmento) }}" enctype="multipart/form-data">
            @csrf @method("put")
            <div class="form-group row">
                <label class="col-sm-2 col-form-label">Codigo</label>
                <div class="col-sm-2">
                    <input type="text" class="form-control @error('id_segmento') is-invalid @enderror" name="id_segmento" value=" {{old('id_segmento', $segmentos->id_segmento ?? '')}}" placeholder="Codigo" readonly>
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label">Nombre</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control @error('nombre_segmento') is-invalid @enderror" name="nombre_segmento" value=" {{old('nombre_segmento', $segmentos->nombre_segmento ?? '')}}" placeholder="Ingrese el Segmento del Proveedor">
                </div>
            </div>
            <hr>
