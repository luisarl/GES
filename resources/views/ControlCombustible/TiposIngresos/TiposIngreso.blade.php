...
@section('titulo_pagina', 'Tipos de Ingreso de Combustible')
...
<ul class="breadcrumb-title">
    ...
    <li class="breadcrumb-item"><a href="#!"> Tipos de Ingresos</a> </li>
</ul>
@endsection
...
@include('ControlCombustible.TiposIngresos.TiposIngresoDestroy')

<!-- Scroll - Vertical table end -->
<!-- Scroll - Vertical, Dynamic Height table start -->
<div class="card">
    <div class="card-header">
        <h5>Listado de Tipos de Ingreso de Combustible</h5>
        

{{-- SECCION DE FILTROS --}}
            @can('cntc.tiposingresos.crear')
                <a class="btn btn-primary  float-right" title="Nuevo" href="{!! route('cntctiposingresos.create') !!}">
        ...
        <div class="table-responsive dt-responsive">
            <table id="datatable" class="table table-striped table-bordered" style="width: 100%">
                ...
                @foreach ($tipos as $tipo)
                    ...
                    <tr>
                        <td>{{$tipo->id_tipo_ingresos}}</td>
                        <td>{{$tipo->descripcion_ingresos}}</td>
                        ...
                        <td class="dropdown">
                            <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog" aria-hidden="true"></i></button>
                                <div class="dropdown-menu dropdown-menu-right b-none contact-menu">
                                    ...
                                    @can('cntc.tiposingresos.editar')
                                        <a class="dropdown-item" href=" {{ route('cntctiposingresos.edit', $tipo->id_tipo_ingresos ) }}"><i class="fa fa-edit"></i>Editar</a>
                                    @endcan
                                    @can('cntc.tiposingresos.eliminar')
                                        <a class="dropdown-item" data-id="{{ $tipo->id_tipo_ingresos}}"
                                        data-nombre="{{ $tipo->descripcion_ingresos }}" data-toggle="modal"
   ...
   <script src="{{ asset('libraries\bower_components\datatables.net\js\jquery.dataTables.min.js') }}"></script>
    ...
    <script>
        $('#modal-eliminar').on('show.bs.modal', function (event) {
                ...
                var modal = $(this)
                modal.find('.modal-body h5').text('Desea Eliminar El Tipo de Ingreso:  ' + nombre + '  ?') //cambia texto en el body del modal
            })
    </script>
